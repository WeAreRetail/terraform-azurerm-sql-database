locals {
  ...
  server_tags_main    = { for n, v in merge(local.tags_main, { "description" = var.server_description }, var.custom_tags) : n => v if v != "" }
  server_tags_dr      = { for n, v in merge(local.tags_dr, { "description" = var.server_description }, var.custom_tags) : n => v if v != "" }
}
